...
void  introducir_datos(char nombreDeUsuario[15],char apellido[15], float *saldoInicial){
...
void jugadas_par(float *saldoInicial) {
	...
	scanf("%f", &dineroApostado);
	while(dineroApostado>*saldoInicial){
		printf("Te has pasado de saldo, vuelve a introducirlo:");
	    scanf("%f", &dineroApostado);
    }
   numeroRuleta= rand()%36;
...
void comprobar (int sumaCartas, float saldoInicial, float saldoJugado){
...
void    blackjack(float *saldoInicial) {
	   ...
	   comprobar(suma, *saldoInicial, saldoJugado);
  }
