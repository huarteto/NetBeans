/***************************************************************************//**
 * @file
 * @brief Energy management unit (EMU) peripheral module library implementation
 *   for EFM32 devices.
...
/***************************************************************************//**
 * @addtogroup EMU
 * @brief EFM32 energy managment unit utilities.
 * @{
 ******************************************************************************/
 ...
 * alter config when waking up from energy modes). It is the responsibility of
 ...
 * @par
 *   Please notice that if the HFXO is re-enabled by this function, and NOT
 *   used to clock the core, this function will not wait for HFXO to stabilize.
 * @par
...
void EMU_EnterEM2(bool restore)
{
  ...
  /* The CMU status should never be 0, except after a reset prior to having */
  /* been set. */
  if (!cmuStatus)
  {
  ...
  if (restore)
  {
    ...
    /* The AUXHFRCO was automatically disabled (except if running debug session) */
    if (cmuStatus & CMU_STATUS_AUXHFRCOENS)
    {
      CMU->OSCENCMD = CMU_OSCENCMD_AUXHFRCOEN;
    }

    /* Restore HF oscillator used */
    ...
    case CMU_STATUS_LFRCOSEL:
      ...
      /* No need to wait for oscillator, not disabled in EM2 */
      break;
    ...
    default: /* CMU_STATUS_HFRCOSEL */
      /* If core clock was HFRCO core clock, it is automatically restored to state */
      /* prior to entering energy mode. */
      break;
    }
 ...
 * @par
 *   Please notice that if the HFXO/LFXO/LFRCO is re-enabled by this function,
 *   and NOT used to clock the core, this function will not wait for those
 *   oscillators to stabilize. This must be considered by the application
 *   if trying to use features relying on those oscillators upon return.
 * @par
 *   If a debugger is attached, the AUXHFRCO will not be disabled if enabled
...
void EMU_EnterEM3(bool restore)
{
  ...
  if (restore)
  {
    ...
    /* Reenable LF oscillators if enabled prior to entering EM3 */
    if (cmuStatus & CMU_STATUS_LFRCOENS)
    {
      CMU->OSCENCMD = CMU_OSCENCMD_LFRCOEN;
    }

    if (cmuStatus & CMU_STATUS_LFXOENS)
    {
