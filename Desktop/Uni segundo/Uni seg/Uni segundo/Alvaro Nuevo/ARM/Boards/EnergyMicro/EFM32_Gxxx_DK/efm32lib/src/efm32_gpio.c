/***************************************************************************//**
 * @file
 * @brief General Purpose IO (GPIO) peripheral library implementation for EFM32
 *   devices.
...
#define GPIO_PORT_VALID(port)        ((port) <= GPIO_PortF)
                     ...
                     unsigned int out)
{
  ...
  {
    pin -= 8;
    GPIO->P[port].MODEH = (GPIO->P[port].MODEH & ~(0xF << (pin * 4))) |
                          (mode << (pin * 4));
  }
...
void GPIO_PortOutSet(GPIO_Port_TypeDef port, uint32_t pins)
{
  ...
  GPIO->P[port].DOUTSET = port & _GPIO_P_DOUTSET_DOUTSET_MASK;
}
