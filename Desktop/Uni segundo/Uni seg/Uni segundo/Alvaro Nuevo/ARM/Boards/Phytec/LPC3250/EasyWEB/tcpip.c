...
#include <LPC325x.h>      // Keil: Register definition file for LPC325x
...
{
// Keil: High Speed Timer is used for TCP retransmission control
  TIMCLK_CTRL   = 0x02;
  HSTIM_MATCH1  = 0;
    ...
    case LISTENING :
    {
      if (TCPFlags & TCP_ACTIVE_OPEN)            // stack has to open a connection?
        if (TCPFlags & IP_ADDR_RESOLVED)         // IP resolved?
          ...
          {
            TCPSeqNr = ((unsigned long)ISNGenHigh << 16) | (HSTIM_COUNTER & 0xFFFF);  // Keil: changed from TAR to T0TC; 
                                                                // set local ISN
...
void TCPClockHandler(void) __irq                 // Keil: interrupt service routine for timer 0
{
  ...
  HSTIM_INT = 0x06;                              // Acknowledge Interrupt
  MIC_RSR   = 0x20;
}
