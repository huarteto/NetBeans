    ...
    case LISTENING :
    {
      if (TCPFlags & TCP_ACTIVE_OPEN)            // stack has to open a connection?
        if (TCPFlags & IP_ADDR_RESOLVED)         // IP resolved?
          ...
          {
            TCPSeqNr = ((unsigned long)ISNGenHigh << 16) | (*AT91C_PITC_PIIR & 0xFFFF);  // Keil: changed from TAR to TIM0->CNTR; 
                                                                // set local ISN
