...
* $Id::$
*
* Project: LPC315x CGU driver
*
* Description:
*     This file contains driver support for the lpc315x CGU.
*
***********************************************************************
...
UNS_32 cgu_fdiv_config(UNS_32 fdId, CGU_FDIV_SETUP_T fdivCfg, BOOL_32 enable)
{
...
void cgu_init_domain_clks(CGU_DOMAIN_CFG_T* pDomain)
{
  ...
  for (fd_id = 0; fd_id < pDomain->fdiv_cnt; fd_id++, psub++)
  {
    ...
    fdiv_conf = cgu_fdiv_config(fd_id + pDomain->fdiv_min, psub->fdiv_cfg, FALSE);
...
void cgu_set_subdomain_freq(CGU_CLOCK_ID_T clkid, CGU_FDIV_SETUP_T fdiv_cfg)
{
  ...
  if (subDomainId != CGU_INVALID_ID)
  {
    ...
    cgu_fdiv_config(subDomainId, fdiv_cfg, TRUE);
    /* enable BCRs */
    if (bcrId != CGU_INVALID_ID)
    {
...
{
  volatile UNS_32 i;
  volatile UNS_32* pmod_rst_reg = &(CGU_CFG->apb1_resetn_soft);
