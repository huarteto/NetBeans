...
//! \b PWM_OUT_3_BIT, \b PWM_OUT_4_BIT, \b PWM_OUT_5_BIT, \b PWM_OUT_6_BIT, or
//! \b PWM_OUT_7_BIT.
//! \param bFaultSuppress determines if the signal is suppressed passed through
...
//!
//! \note This function clears only the FAULT0 interrupt and is retained for
//! backwards compatibility.  It is recommended that function
//! PWMFaultIntClearExt() is used instead of PWMFaultIntClear() since this
//! supports all fault interrupts supported on devices with and without
//! extended PWM fault handling support.
//!
//! \return None.
//
//*****************************************************************************
...
//!
//! \note When running on a device supporting extended PWM fault handling, the
//! fault interrupts are derived by ORing each of the configured fault trigger
//! signals for a given generator.  These interrupts are not, therefore,
//! directly related to the four possible FAULTn inputs to the device but
//! indicate that a fault has been signalled to one of the four possible PWM
