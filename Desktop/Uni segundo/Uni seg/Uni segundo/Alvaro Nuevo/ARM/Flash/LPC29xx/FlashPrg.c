...
int Init (unsigned long adr, unsigned long clk, unsigned long fnc) {
  ...
  CGU_PLL_CTRL = PLL_XTAL_SEL + (15 << MSEL_SHIFT) + P23EN + PLL_PD;
  CGU_PLL_CTRL = PLL_XTAL_SEL + (15 << MSEL_SHIFT) + P23EN;
  while (!(CGU_PLL_STAT & PLL_LOCK   ));  // Wait for PLL Lock
  while (!(CGU_RDET     & PLL_PRESENT));  // Wait for Clock Present
