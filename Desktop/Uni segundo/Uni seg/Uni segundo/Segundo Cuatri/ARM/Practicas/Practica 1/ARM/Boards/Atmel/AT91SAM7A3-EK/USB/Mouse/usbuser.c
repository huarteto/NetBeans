...
#include <AT91SAM7A3.H>                     /* AT91SAM7SA3 definitions */
...
void USB_Reset_Event (void) {
  USB_ResetCore();
  AT91C_BASE_PIOA->PIO_SODR = LED3;         /* Turn Off LED3 */
}
#endif
...
void USB_Suspend_Event (void) {
  AT91C_BASE_PIOA->PIO_CODR = LED4;         /* Turn On LED4 */
}
#endif
...
void USB_Resume_Event (void) {
  AT91C_BASE_PIOA->PIO_SODR = LED4;         /* Turn Off LED4 */
}
#endif
...
void USB_Configure_Event (void) {
  ...
  if (USB_Configuration) {                  /* Check if USB is configured */
    ...
    AT91C_BASE_PIOA->PIO_CODR = LED3;       /* Turn On LED3 */
  } else {
    AT91C_BASE_PIOA->PIO_SODR = LED3;       /* Turn Off LED3 */
  }
}
