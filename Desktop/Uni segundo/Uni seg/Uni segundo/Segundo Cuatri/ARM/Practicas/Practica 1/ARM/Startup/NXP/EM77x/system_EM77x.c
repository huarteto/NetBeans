 ...
 *           for the NXP EM77x Device Series
 * @version  V1.00
 * @date     21. Juky 2010
 *
 * @note
 * Copyright (C) 2010 KEIL(TM) Tools by ARM(R). All rights reserved.
 *
 * @par
...
void SystemFrequencyUpdate (void)            /* Get Core Clock Frequency      */
{
  ...
  switch ((EM77_SYSCON->WDTOSCCTRL >> 5) & 0x0F) {
  ...
  wdt_osc /= ((EM77_SYSCON->WDTOSCCTRL & 0x1F) << 1) + 2;
 
  switch (EM77_SYSCON->MAINCLKSEL & 0x03) {
    case 0:                             /* Internal RC oscillator             */
      SystemFrequency = __IRC_OSC_CLK;
      break;
    case 1:                             /* Input Clock to System PLL          */
      switch (EM77_SYSCON->SYSPLLCLKSEL & 0x03) {
          case 0:                       /* Internal RC oscillator             */
            SystemFrequency = __IRC_OSC_CLK;
            break;
    ...
    case 3:                             /* System PLL Clock Out               */
      switch (EM77_SYSCON->SYSPLLCLKSEL & 0x03) {
          case 0:                       /* Internal RC oscillator             */
            if (EM77_SYSCON->SYSPLLCTRL & 0x180) {
              SystemFrequency = __IRC_OSC_CLK;
            } else {
              SystemFrequency = __IRC_OSC_CLK * ((EM77_SYSCON->SYSPLLCTRL & 0x01F) + 1);
            }
            break;
          case 1:                       /* System oscillator                  */
            if (EM77_SYSCON->SYSPLLCTRL & 0x180) {
              ...
              SystemFrequency = __SYS_OSC_CLK * ((EM77_SYSCON->SYSPLLCTRL & 0x01F) + 1);
            }
            break;
          case 2:                       /* WDT Oscillator                     */
            if (EM77_SYSCON->SYSPLLCTRL & 0x180) {
              ...
              SystemFrequency = wdt_osc * ((EM77_SYSCON->SYSPLLCTRL & 0x01F) + 1);
            }
            break;
          case 3:                       /* Reserved                           */
            SystemFrequency = 0;
  ...
  SystemFrequency /= EM77_SYSCON->SYSAHBCLKDIV;  
...
void SystemInit (void)
{
  ...
  EM77_SYSCON->PDRUNCFG     &= ~(1 << 5);          /* Power-up System Osc      */
  EM77_SYSCON->SYSOSCCTRL    = SYSOSCCTRL_Val;
  for (i = 0; i < 200; i++) __NOP();
  EM77_SYSCON->SYSPLLCLKSEL  = SYSPLLCLKSEL_Val;   /* Select PLL Input         */
  EM77_SYSCON->SYSPLLCLKUEN  = 0x01;               /* Update Clock Source      */
  EM77_SYSCON->SYSPLLCLKUEN  = 0x00;               /* Toggle Update Register   */
  EM77_SYSCON->SYSPLLCLKUEN  = 0x01;
  while (!(EM77_SYSCON->SYSPLLCLKUEN & 0x01));     /* Wait Until Updated       */
#if (SYSPLL_SETUP)                                /* System PLL Setup         */
  EM77_SYSCON->SYSPLLCTRL    = SYSPLLCTRL_Val;
  EM77_SYSCON->PDRUNCFG     &= ~(1 << 7);          /* Power-up SYSPLL          */
  while (!(EM77_SYSCON->SYSPLLSTAT & 0x01));	      /* Wait Until PLL Locked    */
#endif
#endif
#if (WDTOSC_SETUP)                                /* Watchdog Oscillator Setup*/
  EM77_SYSCON->WDTOSCCTRL    = WDTOSCCTRL_Val;
  EM77_SYSCON->PDRUNCFG     &= ~(1 << 6);          /* Power-up WDT Clock       */
#endif
  EM77_SYSCON->MAINCLKSEL    = MAINCLKSEL_Val;     /* Select PLL Clock Output  */
  EM77_SYSCON->MAINCLKUEN    = 0x01;               /* Update MCLK Clock Source */
  EM77_SYSCON->MAINCLKUEN    = 0x00;               /* Toggle Update Register   */
  EM77_SYSCON->MAINCLKUEN    = 0x01;
  while (!(EM77_SYSCON->MAINCLKUEN & 0x01));       /* Wait Until Updated       */
#endif

  EM77_SYSCON->SYSAHBCLKDIV  = SYSAHBCLKDIV_Val;
  EM77_SYSCON->SYSAHBCLKCTRL = AHBCLKCTRL_Val;
  EM77_SYSCON->SSP0CLKDIV    = SSP0CLKDIV_Val;
  EM77_SYSCON->UARTCLKDIV    = UARTCLKDIV_Val;
  EM77_SYSCON->SSP1CLKDIV    = SSP1CLKDIV_Val;
#endif


#if (MEMMAP_SETUP || MEMMAP_INIT)       /* Memory Mapping Setup               */
  EM77_SYSCON->SYSMEMREMAP = SYSMEMREMAP_Val;
#endif
