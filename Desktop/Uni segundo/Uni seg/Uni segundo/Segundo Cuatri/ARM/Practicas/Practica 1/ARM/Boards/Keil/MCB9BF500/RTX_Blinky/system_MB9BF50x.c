...
void SystemInit (void) {
  ...
  MB9BF_CRG->SWC_PSR   = SWC_PSR_Val | (1UL << 7);   /* set SW Watchdog presacaler */
  ...
  if (SCM_CTL_Val & (1UL << 1)) {                    /* Main clock oscillator enabled ? */
    MB9BF_CRG->SCM_CTL |= (1UL << 1);                /* enable main oscillator */ 
    while (!(MB9BF_CRG->SCM_STR & (1UL << 1)));      /* wait for Main clock oscillation stable */
  }
  if (SCM_CTL_Val & (1UL << 3)) {                    /* Sub clock oscillator enabled ? */
    MB9BF_CRG->SCM_CTL |= (1UL << 3);                /* enable sub oscillator */ 
    while (!(MB9BF_CRG->SCM_STR & (1UL << 3)));      /* wait for Sub clock oscillation stable */
  }
  ...
  if (SCM_CTL_Val & (1UL << 4)) {                    /* PLL enabled ? */
    MB9BF_CRG->SCM_CTL  |= (1UL << 4);               /* enable PLL */ 
    while (!(MB9BF_CRG->SCM_STR & (1UL << 4)));      /* wait for PLL stable */
  }
