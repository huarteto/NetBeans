...
HISTORY:   1.24, Pf 28-MAY-04, Support for both Keil and GNU compilers
           1.22, Pf 20-APR-04, FullCAN SW mode implemented
...
// Define when using Keil compiler, undefine for GNU
#define __DKARM

#ifdef __DKARM
// Keil syntax
#define IRQ_KEYWORD __irq
void MCOHW_TimerISR   (void) IRQ_KEYWORD; 
void MCOHW_CANISR_Err (void) IRQ_KEYWORD;
void MCOHW_CANISR_Tx1 (void) IRQ_KEYWORD;
void MCOHW_CANISR_Tx2 (void) IRQ_KEYWORD;
void MCOHW_CANISR_Rx1 (void) IRQ_KEYWORD;
void MCOHW_CANISR_Rx2 (void) IRQ_KEYWORD;
void MCOHW_DefaultISR (void) IRQ_KEYWORD; 
#else // __DKARM
// GNU syntax
...
DWORD TXBufOffset;
  ...
  pAddr = (DWORD *) &C2TFI1 + TXBufOffset;
  *pAddr = candata;  
  ...
  *pAddr = *pCandata;
  ...
  if (TXBufOffset == 0x00)
  {
    C2CMR = 0x21; // Transmission Request  Buf 1
  }
  else if (TXBufOffset == 0x04)
  {
    C2CMR = 0x41; // Transmission Request Buf 2
  }
  else if (TXBufOffset == 0x08)
  {
    C2CMR = 0x81; // Transmission Request Buf 3
  }
  else
  {
    return 0;
