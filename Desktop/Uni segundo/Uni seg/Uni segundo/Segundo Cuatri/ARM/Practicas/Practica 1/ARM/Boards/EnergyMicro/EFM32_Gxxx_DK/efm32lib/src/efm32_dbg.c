/***************************************************************************//**
 * @file
 * @brief Debug (DBG) utilities library implementation for EFM32 devices.
 ...
 *   consumption when debugger is attached, can be avoid when not using
 *   a debugger.
 *
 ...
 *   case, the above suggested code for enabling the SWV pin is not required
 *   in the application.
 *
 * @param[in] location
 *   Pin location used for SWV pin on the application in use.
 ******************************************************************************/
void DBG_SWOEnable(unsigned int location)
{
  ...
  /* TBD: Reconsider GPIO AF_ macro concept */
  ...
  /* Set selected pin location for SWV pin and enable it */
  GPIO_DbgLocationSet(location);
  GPIO_DbgSWOEnable(true);

  /* Configure SWV pin for output */
  GPIO_PinModeSet((GPIO_Port_TypeDef)port, pin, GPIO_ModePushPull, 0);
}
