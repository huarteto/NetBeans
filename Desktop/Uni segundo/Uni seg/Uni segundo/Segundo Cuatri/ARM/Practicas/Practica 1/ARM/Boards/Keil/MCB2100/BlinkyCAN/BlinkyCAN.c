...
HISTORY:   1.24, Pf 28-MAY-04, Support for both Keil and GNU compilers
           1.23, Pf 22-MAR-04, Release for Philips LPC2129
***************************************************************************/ 
  ...
  )  
{
  ...
  // Calculate compare value for time delay
  comp = MCOHW_GetTime() + 10 + (ana >> 2);

  // Variable wait   
  while (!MCOHW_IsTimeExpired(comp)) 
  {
    MCO_ProcessStack();

    // Get new analog input from ADC
    ana = getAD();

    // Put into process image as 1st analog INPUT data
    gProcImg[IN_ana_1]   = (BYTE) ana;
    gProcImg[IN_ana_1+1] = (BYTE) (ana >> 8);

    // Echo analog output received as 2nd analog INPUT data
    gProcImg[IN_ana_2]   = gProcImg[OUT_ana_1];
    gProcImg[IN_ana_2+1] = gProcImg[OUT_ana_1+1];

    // Transmit current timer values in digital INPUTS
