...
//  GPIOC->CRL &= ~0x0000000F;                    // set PIN0 as analog input (see stm32_Init.c)
  ...
  DMA1_Channel1->CMAR  = (u32)current.analog;     // set channel1 memory address
...
void TIM3_IRQHandler (void) {
  ...
  if (TIM3->SR & (1<<0)) {                     /* UIF interrupt ?             */
    ...
    if (current.time.msec == 0) {
      GPIOC->BSRR = (1<<4);                    /* Turn LED#1 On               */
    }
    if (current.time.msec == 100) {
      GPIOC->BRR  = (1<<4);                    /* Turn LED#1 Off              */
    }
  
    if (measurement_interval || mdisplay)  {   /* process measurement         */
      current.gpiod = GPIOD->IDR;              /* Key2, Key3 status           */
