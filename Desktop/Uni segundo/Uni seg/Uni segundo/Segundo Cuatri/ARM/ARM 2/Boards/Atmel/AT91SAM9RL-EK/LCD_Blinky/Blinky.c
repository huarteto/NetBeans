...
const int led_mask[] = { 1 << 15, 1 << 16 }; /* LEDs DS1 and DS2              */
...
extern unsigned char Background_24bpp_rl_fv[];
extern unsigned char ARM_Ani_24bpp_rl_fv[];
...
void LED (unsigned int index, unsigned int state)  {
  if (state) AT91C_BASE_PIOD->PIO_CODR = led_mask[index];
  else       AT91C_BASE_PIOD->PIO_SODR = led_mask[index];
}
...
int main (void) {
  ...
  AT91C_BASE_SYS->SYS_PITC_PIMR  = MCK_FREQ/16/100;
  ...
  GLCD_init();
  GLCD_bitmap (0, 0, 240, 320, Background_24bpp_rl_fv);
  ...
  for (;;) {                            /* Loop forever                       */
    ...
    if (Clock100ms) {
      ...
      if (clk_ani++ == 1) {             /* Draw animation picture every 200 ms*/
        clk_ani = 0;
        if (pic++ > 8) pic = 0;
        GLCD_bitmap (99, 99, 45, 120, &ARM_Ani_24bpp_rl_fv[pic*(((45*3+3)&~0x03)*120)]);
      }
    }
