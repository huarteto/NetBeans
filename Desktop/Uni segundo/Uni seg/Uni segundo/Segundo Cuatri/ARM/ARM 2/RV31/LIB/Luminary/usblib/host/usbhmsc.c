           ...
           USB_EP_ATTR_BULK)
        {
            ...
            if(pEndpointDescriptor->bEndpointAddress & USB_EP_DESC_IN)
            {
                ...
                g_USBHMSCDevice.ulBulkInPipe =
                    USBHCDPipeAlloc(0, USBHCD_PIPE_BULK_IN, pDevice->ulAddress,
                                    0);
                //
            ...
            else
            {
                ...
                g_USBHMSCDevice.ulBulkOutPipe =
                    USBHCDPipeAlloc(0, USBHCD_PIPE_BULK_OUT,
                                    pDevice->ulAddress, 0);
                //
...
//!
//! This function checks if the current device has become ready to be accessed.
//! It uses the \e ulInstance parameter to determine which device to access and
//! will return zero when the device is ready.  Any negative return code
//! indicates that the device was not ready.
//!
//! \return This function will return zero if the device is ready and it will
//! return a negative value if the device is not ready or if an error occurred.
//
//*****************************************************************************
long
                 ...
                 unsigned char *pucData, unsigned long ulNumBlocks)
{
    ...
    if(USBHSCSIRead10(pMSCDevice->ulBulkInPipe, pMSCDevice->ulBulkOutPipe,
                      ulLBA, pucData, &ulSize) != SCSI_CMD_STATUS_PASS)
    {
        return(-1);
    }
