...
void SystemCoreClockUpdate (void)           /* Get Core Clock Frequency */
{
  ...
  if (TSB_CG->CKSEL & (1 << 1)) {           /* If system clock is low-speed clock  */
...
void SystemInit(void)
{
  ...
  if (!(TSB_WD->MOD & (1 << 7))) {          /* If watchdog is to be disabled */
    TSB_WD->CR = WDCR_Val;
  }
#endif
  ...
  if (TSB_CG->OSCCR  & (1 << 2)) {          /* If PLL enabled       */
    TSB_CG->OSCCR  |=  1;                   /* Enable Warm-Up Timer */
    while(TSB_CG->OSCCR & 0x02) {}          /* Wait for completion  */
  }
